...
export function ExecutionStep({ draft, onChange }: { draft: StrategyWizardDraft; onChange: (draft: StrategyWizardDraft) => void }) {
  ...
  return <div className="strategy-wizard-section">
    ...
    <div className="strategy-toggle-list">
      ...
      {draft.definition.marketType === "FUTURES" ? <Toggle label="Trades stay perpetually open" text="A position remains open until an exit, reversal or protection rule closes it." checked={execution.perpetuallyOpen === true} onChange={(value) => patch("perpetuallyOpen", value)} /> : null}
    </div>
    <div className="strategy-form-grid">
      ...
      <Select label="CONFLICT RESOLUTION" value={String(execution.conflictResolution || "CLOSE_THEN_REVERSE")} options={[["IGNORE", "Ignore Conflict"], ["CLOSE_ONLY", "Close Only"], ["CLOSE_THEN_REVERSE", "Close, Then Reverse"]]} onChange={(value) => patch("conflictResolution", value)} />
    </div>
