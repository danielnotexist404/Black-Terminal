...
- Moved Volume Profile annotations into an independent overlay so high-resolution sub-pixel bars cannot clip price and POC/HVN text.
- Restored the complete dense Volume Profile price/classification columns through that unclipped overlay.
...
## 2026-08-23 — BC-RDA repaint containment and Causal V2 reconstruction

Reproduced the legacy moving-trough backpaint, classified it as `BC_RDA_LEGACY_REPAINTING`, invalidated its historical signal statistics, and disabled every alert, Strategy Lab, Event Alpha, Paper, Demo, group and live execution path. Added separately versioned `BC_RDA_CAUSAL_V2` with explicit developing/final lifecycle timestamps, confirmation-bar rendering, execution eligibility no earlier than bar close, fixed warm-up/lookback semantics, point-in-time metadata and configuration-bound checkpoints. The deterministic Native/Compatibility source harness passed 180 fixed prefixes, 100 random truncations, future appends through 1,000 bars, and 40 worker/reload/checkpoint parity cases each with zero finalized drift. A new migration contains persisted BC-RDA runtimes but was not deployed. Source certification does not authorize alerts or automation; both remain blocked pending a distinct headless VPS-runtime chapter.

## 2026-08-23 — BC-QALC Research foundation

Added the native event-time BC-QALC engine: Bybit depth/trade Research gateway, atomic book, sequencing/clock gates, real-flow feature registry, separate direction/fill/adverse-selection models, dynamic instrument and account-fee adapters, all-in cost rejection, conservative one-sided PostOnly Paper simulator, event archive/replay, risk engine, authenticated metadata API, Docker worker and dedicated Strategy Lab wizard/cockpit. Local and exact-image correctness plus 20,000-event internal performance gates pass. The portless, credential-isolated BTCUSDT Research capture is deployed to the VPS preview with migration `202608230004`; live/group execution and Paper quoting remain disabled. Certification remains RESEARCH; elapsed capture/forward/soak gates, calibration and platform-wide raw-stream unification remain pending.
