...
- LAN discovery uses mDNS. After one trusted peer is reached, an isolated Black Terminal Kademlia/Identify mesh expands discovery across reachable peers; the native node also attempts UPnP port mapping, exposes any verified external multiaddress, and automatically reconnects encrypted locally remembered trusted addresses. First contact still requires a shared trusted multiaddress, and internet-wide reachability behind symmetric NAT or restrictive firewalls requires an operator-configured relay/rendezvous service; none is silently supplied by the VPS.
