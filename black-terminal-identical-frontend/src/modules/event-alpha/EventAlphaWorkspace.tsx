...
function EventAlphaWorkspaceContent({ onClose }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    setMessage(failures.length ? `${results.length - failures.length}/${results.length} evidence surfaces synchronized · ${failures.length} unavailable` : "All server-authoritative evidence surfaces synchronized.");
    setRefreshing(false);
  }, []);
...
function ControlsPanel({ config }: { config: EventAlphaRuntimeConfig | null }) { const controls = [["Engine rollout", config?.engineEnabled, "EVENT_ALPHA_ENGINE_ENABLED"], ["Crypto ingestion", config?.ingestionEnabled, "server worker"], ["Equity PEAD engine", config?.equityPeadEnabled, "EVENT_ALPHA_EQUITY_PEAD_ENABLED"], ["PEAD normalized provider", config?.peadProviderConfigured, "consensus + actuals + factor returns"], ["Paper execution", config?.paperExecutionEnabled, "separate approval boundary"], ["Manual approval", config?.manualApprovalRequired, "always required"]] as const; return <div className="event-alpha-controls"><div className="event-alpha-warning"><AlertTriangle size={17} /><div><b>FAIL-CLOSED RESEARCH CONTROL PLANE</b><p>Data controls are server environment policy. A missing consensus, filing, market path or causal timestamp produces NO_TRADE.</p></div></div>{controls.map(([label, enabled, note]) => <div key={label}><span>{label}</span><b className={enabled ? "on" : "off"}>{enabled ? "ENABLED" : "DISABLED"}</b><small>{note}</small></div>)}</div>; }

class EventAlphaErrorBoundary extends Component<{ children: ReactNode; onClose: () => void }, { failed: boolean }> { state = { failed: false }; static getDerivedStateFromError() { return { failed: true }; } componentDidCatch(error: Error, info: ErrorInfo) { console.error("[event-alpha-ui-boundary]", { name: error.name, componentStack: info.componentStack }); } render() { if (!this.state.failed) return this.props.children; return <section className="event-alpha-workspace event-alpha-recovery"><div><AlertTriangle size={22} /><b>EVENT ALPHA DISPLAY RECOVERED</b><p>The client projection failed. Server evidence was not changed.</p><button type="button" onClick={() => this.setState({ failed: false })}>RETRY WORKSPACE</button><button type="button" onClick={this.props.onClose}>RETURN TO CHART</button></div></section>; } }
