...
assert.equal((draftStore.match(/^\s*"(?:Identity|Indicator and Market|Signal Mapping|Execution Behavior|Risk Management|Filters and Schedule|Take Profits and Exits|Paper Account|Bybit Demo Account|Activate Strategy & Save Configuration)",?$/gm) || []).length, 10);
...
assert.match(targetMatrix, /No Bybit Demo account allocated/);
assert.match(targetMatrix, /bindings\.filter/, "only occupied bindings create detailed state");
assert.match(targetsStep, /api-demo\.bybit\.com/);
assert.match(targetsStep, /type="password"/);
assert.match(targetsStep, /setApiSecret\(""\)/, "demo secret is cleared after one-shot submission");
...
console.log("My Strategy UX tests PASS — private activation naming, Bybit Demo credential boundary, ten-step workflow, versioning, cockpit navigation and target rendering verified.");
