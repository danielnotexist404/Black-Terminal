...
- Local email delivery uses an encrypted durable outbox and a native SMTP client. SMTP usernames and application passwords/tokens stay in the operating-system credential vault; provider settings and pending messages are encrypted locally. Only authenticated TLS-wrapper or mandatory STARTTLS submission is accepted, and no email is routed through Black Cloud.
