...
export default function BlackGPT({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!API_KEY) {
        throw new Error("Gemini API key is not configured in .env variables.");
      }
...
`;
      ...
      if (!response.ok) {
        ...
        throw new Error(`Gemini API connection error: ${errorText}`);
      }

      const resData = await response.json();
      const modelText = resData.candidates?.[0]?.content?.parts?.[0]?.text || "No response generated by model.";
