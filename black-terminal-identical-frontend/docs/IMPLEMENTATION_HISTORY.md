...
Added the native event-time BC-QALC engine: shared Bybit depth/trade gateway, atomic book, sequencing/clock gates, real-flow feature registry, separate direction/fill/adverse-selection models, dynamic instrument and account-fee adapters, all-in cost rejection, conservative one-sided PostOnly Paper simulator, event archive/replay, risk engine, authenticated metadata API, Docker worker and dedicated Strategy Lab wizard/cockpit. Local correctness and 20,000-event internal performance gates pass. Certification remains RESEARCH; elapsed capture/forward/soak gates, calibration and VPS deployment are not yet evidence.
