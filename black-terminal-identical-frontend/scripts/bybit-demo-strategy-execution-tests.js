...
assert.match(signalWorker, /takeProfitPlan\.slice\(0, 7\)/, "the signal evaluator durably queues every effectively reserved target order");
assert.match(signalWorker, /shouldQueueStrategyTakeProfits\(action\)/, "entry and reverse actions both install the new position's target ladder");
