...
export function PaperStep({ draft, onChange }: { draft: StrategyWizardDraft; onChange: (draft: StrategyWizardDraft) => void }) {
  ...
  return <div className="strategy-wizard-section"><header><span>08</span><div><h2>Paper account and backtest model</h2><p>Configure the isolated simulation used for validation and backtesting. It becomes the active destination only when Paper Backtester is explicitly selected in the next step.</p></div></header>
    <div className="paper-account-hero"><WalletCards size={24} /><div><strong>PAPER MODEL</strong><span>Separate virtual account · {draft.definition.marketType} · Never connected to real funds</span></div><b>AVAILABLE</b></div>
