...
Investment Group join drafts and operational membership state are server records, not workspace/localStorage state. The UI can navigate to Positions for broker setup and later resume the server draft. The cockpit uses one bounded group refresh and canonical snapshots rather than a browser polling loop per member.
