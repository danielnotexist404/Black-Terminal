    ...
    <section className="institutional-flow panel-block" aria-label="Institutional flow intelligence">
      ...
      {snapshot?.state === "unsupported" ? (
        ...
        <>
          ...
          <PressureOscillator points={snapshot.oscillator} current={pressure} />
...
function PressureOscillator({ points, current }: { points: InstitutionalFlowPoint[]; current: number }) {
  const model = useMemo(() => oscillatorModel(points), [points]);
  return (
    <div className="institutional-pressure" title="Black Core institutional pressure oscillator. White/silver is positive secondary-market pressure; blood red is negative. It does not claim primary ETF flow.">
      ...
      <svg viewBox="0 0 320 92" preserveAspectRatio="none" role="img" aria-label={`Institutional pressure ${current.toFixed(1)}`}>
        <defs>
...
function oscillatorModel(points: InstitutionalFlowPoint[]) {
  ...
  void latestY;
  return { pressure, signal: path("signal"), area, bars };
}
