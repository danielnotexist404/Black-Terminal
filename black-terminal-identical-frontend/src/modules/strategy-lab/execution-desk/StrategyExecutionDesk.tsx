...
function ExecutionDeskSurface({
  ...
  return <section className="execution-desk">
    ...
    {settingsOpen && selectedPolicy && onApplyConfiguration && superAtrControlsAvailable ? <StrategyControlPanelDialog name={strategy.name} accountLabel={selected.label} initial={controlPanel} sourceKey={selected.key} authoritativeDestination={Boolean(selected.binding)} authoritativeEquity={selected.binding ? selected.snapshot?.equity : undefined} authoritativeAvailableBalance={selected.binding ? selected.snapshot?.availableBalance : undefined} authoritativeEquityTimestamp={selected.binding ? selected.snapshot?.timestamp : undefined} authoritativeFreshness={selected.binding ? selected.snapshot?.freshness : undefined} busy={busy} onCancel={() => setSettingsOpen(false)} onApply={async (panel) => { await onApplyConfiguration(strategy.definition, selectedPolicy, selected.key, panel); setSettingsOpen(false); }} /> : null}
  </section>;
}
