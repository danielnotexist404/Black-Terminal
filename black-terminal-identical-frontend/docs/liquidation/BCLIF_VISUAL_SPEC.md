...
In `BCLIF_MODEL_V3`, confidence-weighted exposure is weighted before robust quantile normalization. Confidence therefore changes relative rank and remains available as an independent field, while the strongest supported exposure can still occupy the green/yellow end of the reference palette. Applying confidence after normalization is prohibited because it collapses the available color range whenever aggregate confidence is below 100%.
