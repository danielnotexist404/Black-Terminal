...
export function migrateDomPanelRegistry(input: unknown, workspaceId: string, symbolKey: string): DomPanelSettingsRegistry {
  ...
  if (sourceVersion < 7) {
    ...
    ladder.settings.levels = Math.max(64, Number(ladder.settings.levels) || 0);
    ladder.settings.updateIntervalMs = Math.min(250, Math.max(100, Number(ladder.settings.updateIntervalMs) || 250));
