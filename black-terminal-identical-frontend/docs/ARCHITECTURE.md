...
Bybit account synchronization performs a guarded paginated REST open-order snapshot before publishing account order state. The persistent private worker consumes the all-category `order` topic and schedules reconciliation for every order lifecycle change. REST and stream records normalize to `network + connection + venue + category + venueOrderId`; externally created orders are retained without a local submission record. Black Core owns the keyed active-order set consumed by both the Orders panel and current-symbol chart overlay. The chart overlay shares the Pixi host origin and authoritative price transform. See `BYBIT_ORDER_SYNC_ARCHITECTURE.md` and `BYBIT_CANONICAL_ORDER_IDENTITY.md`.

For Bybit, canonical connection identity is the venue user ID with a server-only API-key fingerprint fallback. Local Supabase account rows are persistence records, not venue identity. Snapshot loading collapses legacy duplicate records before portfolio totals; disconnect deletes credential-equivalent records and authoritative account reconciliation purges their orders from every consumer. See `BYBIT_CONNECTION_LIFECYCLE_HOTFIX.md`.

## DOM Pro Shared Price Domain

The Aggregated DOM Ladder, Volume Profile and Liquidity Heatmap consume one `DomProPriceCamera` in Shared mode. The camera creates exact visible buckets and normalized Y geometry; consumers aggregate their own truthful dataset into those buckets without deriving another range. Current live ladder coverage is explicitly separated from wider IMM history. Uncovered prices render unavailable rather than false zero depth. `DomFeedStore` remains the single feed owner, and the pure ladder model cannot subscribe to a venue. See `DOM_PRO_SHARED_PRICE_CAMERA.md`, `DOM_PRO_AGGREGATED_LADDER_SYNC.md` and `DOM_PRO_LIVE_DEPTH_COVERAGE.md`.
