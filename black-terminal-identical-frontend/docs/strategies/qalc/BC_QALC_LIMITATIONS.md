...
- `RESEARCH LONG/SHORT` and `MODEL TP/INVALIDATION` are causal model projections, not filled positions or resting broker orders. `PAPER ENTRY/EXIT` remains absent until a certified Paper runtime records conservative queue fills.
- Fill probability uses gross side-resolved aggressive flow to estimate queue consumption; net signed OFI remains a separate directional feature and must not be substituted for executed opposing volume.
