...
- Broker credentials are stored as v2 envelope-encrypted records and are decrypted only in the worker.
...
The architecture and local contract tests are implemented. An always-on external host, applied
Chapter II-B migration, broker test credentials, and recorded browser-independent testnet evidence
are external prerequisites. Until those exist, the truthful state is `PERSISTENT CAPABLE — NOT
CERTIFIED`, not `FULLY PERSISTENT`.
