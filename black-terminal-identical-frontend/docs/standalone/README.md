...
- LAN discovery uses mDNS. After one trusted peer is reached, an isolated Black Terminal Kademlia/Identify mesh expands discovery across reachable peers; the native node also attempts UPnP port mapping, exposes any verified external multiaddress, and automatically reconnects encrypted locally remembered trusted addresses. First contact still requires a shared trusted multiaddress. Internet-wide reachability behind symmetric NAT or restrictive firewalls uses operator-configured Circuit Relay v2 reservations and DCUtR direct-upgrade attempts. The hardened relay container and operating runbook live in `infra/p2p-relay`; a live relay is never silently supplied by the VPS.
